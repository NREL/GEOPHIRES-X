...
class SUTRAWellBores:
    ...
    def Calculate(self, model: Model) -> None:
        ...
        rhowaterprod = 1000
        muwaterprod = 1E-3
        vprod = abs(prodwellflowrates) / rhowaterprod / (math.pi / 4. * self.prodwelldiam.value ** 2)
        Rewaterprod = 4. * abs(prodwellflowrates) / (muwaterprod * math.pi * self.prodwelldiam.value)
        for i in range(len(Rewaterprod)):
            if vprod[i] == 0:
                ...
                if Rewaterprod[i] <2300:
                    ...
                    f = 1. / np.power(-2 * np.log10(relroughness / 3.7 + 5.74 / np.power(Rewaterprod[i], 0.9)), 2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod[i] / np.sqrt(f))), 2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod[i] / np.sqrt(f))), 2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod[i] / np.sqrt(f))), 2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod[i] / np.sqrt(f))), 2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod[i] / np.sqrt(f))), 2.)

                self.DPProdWell.value[i] = f * (rhowaterprod * vprod[i] ** 2 / 2) * (model.reserv.depth.value / self.prodwelldiam.value) / 1E3  # /1E3 to convert from Pa to kPa

        # calculate pressure drop injection well [kPa]
        self.DPInjWell.value = np.zeros(len(model.reserv.timevector.value))
        rhowaterinj = 1000
        muwaterinj = 1E-3
        vinj = abs(injwellflowrates) / rhowaterinj / (
                math.pi / 4. * self.injwelldiam.value ** 2)
        Rewaterinj = 4. * abs(injwellflowrates) / (
                muwaterinj * math.pi * self.injwelldiam.value)
        for i in range(len(Rewaterinj)):
            if vinj[i] == 0:
                ...
                if Rewaterinj[i] < 2300:
                    ...
                    f = 1. / np.power(-2 * np.log10(relroughness / 3.7 + 5.74 / np.power(Rewaterinj[i], 0.9)),
                                      2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterinj[i] / np.sqrt(f))),
                                      2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterinj[i] / np.sqrt(f))),
                                      2.)
                    f = 1. / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterinj[i] / np.sqrt(f))),
                ...
                self.DPInjWell.value[i] = f * (rhowaterinj * vinj[i] ** 2 / 2) * (
                        model.reserv.depth.value / self.injwelldiam.value) / 1E3  # /1E3 to convert from Pa to kPa

        # Calculate buoyancy pressure drop [kPa]
        DP_buoyancy = (rhowaterprod - rhowaterinj) *  model.reserv.depth.value * 9.81 / 1E3  # /1E3 to convert from Pa to kPa
