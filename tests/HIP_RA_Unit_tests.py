...
import pytest, sys, os, logging

from geophires_x.Parameter import floatParameter, intParameter, OutputParameter
from geophires_x.Units import VolumeUnit, HeatUnit, MassUnit, EnthalpyUnit, PercentUnit, PowerUnit, Units, \
    TemperatureUnit
from geophires_x.GeoPHIRESUtils import DensityWater, celsius_to_kelvin, ViscosityWater, HeatCapacityWater, RecoverableHeat, \
    VaporPressureWater, interp_entropy_func, EntropyH20_func, EnthalpyH20_func, interp_util_eff_func, UtilEff_func
from hip_ra.HIP_RA import HIP_RA


class TestDensityWater:
...
class TestCelsiusToKelvin:
    ...
    #  Should raise a ValueError when given a negative Celsius value
...
class TestHipRa:
    ...
    def test_convert_units_of_output_parameters(self):
        hip_ra = HIP_RA()
        hip_ra.OutputParameterDict['Reservoir Volume'].CurrentUnits = VolumeUnit.METERS3
        hip_ra.OutputParameterDict['Stored Heat'].CurrentUnits = HeatUnit.J
        hip_ra.OutputParameterDict['Fluid Produced'].CurrentUnits = MassUnit.GRAM
        hip_ra.OutputParameterDict['Enthalpy'].CurrentUnits = EnthalpyUnit.KJPERKG
        hip_ra.OutputParameterDict['Wellhead Heat'].CurrentUnits = HeatUnit.J
        hip_ra.OutputParameterDict['Recovery Factor'].CurrentUnits = PercentUnit.PERCENT
        hip_ra.OutputParameterDict['Available Heat'].CurrentUnits = HeatUnit.J
        hip_ra.OutputParameterDict['Producible Heat'].CurrentUnits = HeatUnit.J
        hip_ra.OutputParameterDict['Producible Electricity'].CurrentUnits = PowerUnit.W
        hip_ra.PrintOutputs()
        assert hip_ra.OutputParameterDict['Reservoir Volume'].CurrentUnits == VolumeUnit.KILOMETERS3
        assert hip_ra.OutputParameterDict['Stored Heat'].CurrentUnits == HeatUnit.KJ
        assert hip_ra.OutputParameterDict['Fluid Produced'].CurrentUnits == MassUnit.KILOGRAM
        assert hip_ra.OutputParameterDict['Enthalpy'].CurrentUnits == EnthalpyUnit.KJPERKG
        assert hip_ra.OutputParameterDict['Wellhead Heat'].CurrentUnits == HeatUnit.KJ
        assert hip_ra.OutputParameterDict['Recovery Factor'].CurrentUnits == PercentUnit.PERCENT
        assert hip_ra.OutputParameterDict['Available Heat'].CurrentUnits == HeatUnit.KJ
        assert hip_ra.OutputParameterDict['Producible Heat'].CurrentUnits == HeatUnit.KJ
        assert hip_ra.OutputParameterDict['Producible Electricity'].CurrentUnits == PowerUnit.MW
